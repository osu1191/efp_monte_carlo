...
*/
/*
void ANIModel::testDouble_engrad() {
    ...
    torch::jit::script::Module aev_computer = torch::jit::load("/depot/lslipche/data/skp/libtorch/aev_scripted.pt");

    auto species = torch::tensor({3, 0, 0}, torch::dtype(torch::kInt64));
    auto coordinates = torch::tensor({
    ...
    auto elecpots = torch::tensor({0.0, 0.0, 0.0}, torch::dtype(torch::kDouble));

    std::unordered_map<int, double> ani1x_sae_dict_byIdx = {
        {0, -0.60095298}, {1, -38.08316124}, {2, -54.7077577}, {3, -75.19446356}
    };
    double shift = 0.0;
    for (int i = 0; i < species.size(0); ++i) {
        shift += ani1x_sae_dict_byIdx[species[i].item<int>()];
    }
    ...
    auto aev_input = torch::ivalue::Tuple::create({species, coordinates});
    auto aev_output = aev_computer.forward({aev_input}).toTuple();
    auto aevs = aev_output->elements()[1].toTensor();

    auto elecpots_unsqueezed = elecpots.unsqueeze(-1);
    auto aep = torch::cat({aevs, elecpots_unsqueezed}, -1);

    auto model_input = torch::ivalue::Tuple::create({species, aep});
    ...
    auto derivative = torch::autograd::grad({energy_shifted}, {coordinates}, {}, true, true)[0];
    auto analytical_grad = derivative * 0.529177; // Convert to atomic units
    auto force = -analytical_grad;

    std::cout << "Energy unshifted: " << energy_unshifted.item<double>() << std::endl;
    std::cout << "Energy shifted: " << energy_shifted.item<double>() << std::endl;
    std::cout << "Force: " << force << std::endl;
...
void ANIModel::get_custom_energy_grad(float* coordinates_data, int64_t* species_data, float* elecpots_data, int num_atoms, double* custom_energy, float* cus_grads, float* cus_forces, int print) {
    ...
    std::cout << "====================== Running testDouble_engrad() ======================" << std::endl;
    testDouble_engrad();    
    std::cout << "====================== Done running testDouble_engrad() ======================" << std::endl;
...
void ANIModel::get_custom_energy_grad(float* coordinates_data, int64_t* species_data, float* elecpots_data, int num_atoms, double* custom_energy, float* cus_grads, float* cus_forces, int print) {
    ...
    coordinates = coordinates.to(torch::kDouble);
    elecpots = elecpots.to(torch::kDouble);
...
//    memcpy(cus_forces, force.data_ptr<float>(), force.numel() * sizeof(float));
    ...
    memcpy(cus_grads, derivative.to(torch::kFloat32).data_ptr<float>(), derivative.numel() * sizeof(float));
    memcpy(cus_forces, force.to(torch::kFloat32).data_ptr<float>(), force.numel() * sizeof(float));
    *custom_energy = static_cast<double>(energy_shifted.item<double>());
}
